...
int main(int argc, char *argv[])
{
    ...
    while ((ch = getopt_long(argc, argv, short_opt, long_opt, nullptr)) != -1)
    {
        ...
        case 'h':
            ...
            std::cout << "-k \t \t \t Prints measurements from the keyboard directly. Only recommended in single-measurement mode (otherwise it'll be noisy)" << std::endl;
            return 0;
    ...
    while (true)
    {
        ...
        else if (bytesRead > 1)
        {
            ...
            if (measurementObj.getNativeMeasurement() == 0.0)
            {
